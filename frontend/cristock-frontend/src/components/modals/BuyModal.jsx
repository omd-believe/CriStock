...
export default function BuyModal({ player, isOpen, onClose, onSuccess }) {
  ...
  const handleConfirm = async () => {
    ...
    if (total > available) { setError('Insufficient funds'); return; }
    if (safeQty > player.sharesAvail) { setError('Not enough shares available in market'); return; }

    setLoading(true);
    try {
      const result = await buyShares(Number(player.id), safeQty);
   
      updateWalletBalance(available - Number(result.totalAmount || total));
      onSuccess(`Bought ${safeQty} shares of ${player.name} at ₹${player.price.toFixed(2)}`);
      onClose();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-[#1A1A26] rounded-2xl p-6 w-full max-w-md border border-white/10 relative">
        ...
        <div className="mb-6">
          <h2 className="text-xl font-bold text-white mb-1">Buy {player.name}</h2>
          <p className="text-[#00FF87] font-mono text-lg">₹{player.price.toFixed(2)}</p>
        </div>
        ...
        <div className="mb-6">
          ...
          <input
            ...
            className="w-full bg-[#0A0A0F] border border-white/10 text-white font-mono rounded-lg px-4 py-3 outline-none focus:border-[#00FF87] transition-colors"
          />
          <p className="text-white/40 text-xs mt-2 text-right">
            {player.sharesAvail.toLocaleString('en-IN')} shares available in market
          </p>
        </div>
        ...
        <div className="space-y-2 mb-6">
          ...
          <div className="flex justify-between text-sm py-2 border-y border-white/5 my-2">
            <span className="text-white/60 font-medium">Total cost</span>
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className={`font-mono ${afterTrade < 0 ? 'text-[#FF4757]' : 'text-white/80'}`}>
              ₹{afterTrade.toFixed(2)}
            </span>
          </div>
        </div>
        ...
        <button
          ...
          disabled={loading || afterTrade < 0 || safeQty < 1}
          className={`w-full bg-[#00FF87] text-black font-semibold rounded-lg py-3 transition-all ${
            loading || afterTrade < 0 || safeQty < 1
              ? 'opacity-60 cursor-not-allowed'
              : 'hover:bg-[#00E87A] active:scale-[0.98]'
          }`}
        >
          {loading ? 'Processing...' : `Confirm Buy ${safeQty > 0 ? `— ₹${total.toFixed(2)}` : ''}`}
        </button>
      </div>
    </div>
  );
